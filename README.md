...
The server-rendered portal is runnable locally with Google search, a responsive D1-backed favorites grid, and a single-password authenticated admin area. The admin can add validated favorites with automatic metadata/icon previews or generated fallback icons through either normal HTML or an HTMX-enhanced dashboard flow. Custom icon uploads backed by R2 are the next implementation phase. See [PLAN.md](PLAN.md) for the active checklist, roadmap, decisions, and definition of done.
